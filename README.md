...
本项目**主要面向复杂城市环境下的 GNSS 多径（multipath）研究**：在软件园2期、湖边万达、五缘湾、厦禾路四个真实城市场景同步采集 GNSS 观测、参考轨迹与视觉数据，覆盖商业综合体、城市街谷、滨海开阔区等典型多径传播条件，并提供定位基准测试、应用场景分类、多径分类，以及基于相关器 I/Q 的 SAGE 多径参数估计与定位扩展代码。
